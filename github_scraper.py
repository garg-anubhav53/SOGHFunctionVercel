...
class GithubScraper:
    ...
    def load_cookies(self):
        ...
        try:
            ...
            if cookies_str:
                ...
                self.session.cookies.update(cookies_dict)
                logger.info("Cookies loaded successfully from .env file")
            else:
                logger.warning("No GITHUB_COOKIES found in .env file")
        except json.JSONDecodeError as e:
            logger.error(f"Error parsing cookies JSON from .env: {e}")
        except Exception as e:
    ...
    def get_github_link(self, so_url):
        ...
        try:
            logger.info(f"Processing: {so_url}")
            # Ensure URL starts with https://
            if not so_url.startswith('http'):
                full_url = f"https://{so_url}"
            else:
                full_url = so_url
            ...
            logger.warning(f"No GitHub profile link found in {full_url}")
            return None
            
        except Exception as e:
            logger.error(f"Error getting GitHub link: {e}")
            return None
    ...
    def get_stackoverflow_info(self, so_url):
        ...
        try:
            ...
            for link in all_links:
                ...
                # Skip Stack Overflow company social media links
                if any(x in href for x in [
                    'twitter.com/stackoverflow',
                    'x.com/stackoverflow',
                    'instagram.com/stackoverflow',
                    '/company/stack-overflow'
                ]):
                    continue
                    
                if 'twitter.com' in href or 'x.com' in href:
                    profile_info['twitter'] = href
                elif 'github.com' in href and '/users/' not in href:  # Exclude SO user links
                    profile_info['github'] = href
                elif any(x in href for x in ['blog', '.io', '.com', '.org', '.net']):
                    if not any(x in href for x in ['stackoverflow.com', 'stackexchange.com', 'twitter.com', 'x.com', 'github.com']):
                        profile_info['blog'] = href
            
            # Get bio/about me using s-prose class
            about_me = soup.find('div', {'class': 's-prose'})
            if about_me:
                profile_info['bio'] = about_me.get_text(strip=True)
            
            # Get basic profile info
            name_elem = soup.find('div', {'class': 'fs-headline2'})
            if name_elem:
                profile_info['name'] = name_elem.get_text(strip=True)
            
            # Get stats from the flex container
            stats_sections = soup.find_all('div', {'class': 'flex--item'})
            for section in stats_sections:
                # Look for the value and label divs
                value_div = section.find('div', {'class': 'fs-body3'})
                if not value_div:
                    continue
                
                value = value_div.get_text(strip=True)
                
                # Find the label text that follows the value
                label_text = None
                next_elem = value_div.find_next_sibling(text=True)
                if next_elem:
                    label_text = next_elem.strip().lower()
                
                if label_text:
                    if 'reputation' in label_text:
                        profile_info['stats']['reputation'] = value
                    elif 'reached' in label_text:
                        profile_info['stats']['reached'] = value
                    elif 'answers' in label_text:
                        profile_info['stats']['answers'] = value
                    elif 'questions' in label_text:
                        profile_info['stats']['questions'] = value
            
            # Get location if available
            location_section = soup.find('div', string=lambda x: x and 'Location:' in str(x))
            if location_section:
                next_elem = location_section.find_next()
                if next_elem:
                    profile_info['location'] = next_elem.get_text(strip=True)
                    ...
                    try:
                        ...
                        if github_info and so_info:
                            ...
                            csv_writer.writerow([
                                ...
                                self.sanitize_csv_field(github_info.get('email')),
                                self.sanitize_csv_field(github_info.get('bio')),
                                self.sanitize_csv_field(github_info.get('name')),
                                self.sanitize_csv_field(github_info.get('username')),
                                self.sanitize_csv_field(github_info.get('location')),
                                self.sanitize_csv_field(github_info.get('company')),
                                self.sanitize_csv_field(github_info.get('website')),
                                self.sanitize_csv_field(github_info.get('followers')),
                                self.sanitize_csv_field(github_info.get('following')),
                                self.sanitize_csv_field(github_info.get('contributions')),
                                self.sanitize_csv_field(json.dumps(github_info.get('pinned_repositories'))),
                                ...
                                self.sanitize_csv_field(so_info.get('bio')),
